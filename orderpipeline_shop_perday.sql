Select  
 		...
 		cast(st.createddatetime as date) as "CreatedDate",
...
Where 
		...
		and st.salesoriginid in ('TMALL_CN')
		--and st.salesoriginid in ('BEBITUS_ES','BEBITUS_FR','BEBITUS_PT')


Group by st.salesoriginid,cast(st.createddatetime as date)--,st.winsplitordercounter
order by st.salesoriginid,cast(st.createddatetime as date)
